...
def generate_framework_def(
    username: str,
    branch: str,  # Technically this is not used
    commit: str,
):
    ...
    # Include the tag in the config.yaml so we can access our framework
    config = None
    config_path = './resources/config.yaml'
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)

    framework_name = f"{username}_{branch}_{commit}"
    config['frameworks']['tags'].append(framework_name)
    ...
    lines = [
        ...
        f"autosklearn:\n",
        f"  version: '{commit}'\n",
        f"  repo: '{f'git@github.com:{username}/auto-sklearn.git'}'\n"
    ]
    with open(f'./resources/{framework_name}.yaml', 'w') as f:
        f.writelines(lines)
...
def create_comparison(
    ...
    * ['id', 'task', 'framework', 'constraint', 'fold', 'type', 'result',
    ...
    ----------
    baseline_csv: path
        Path to the csv containing the baseline results

    targeted_csv: path
        Path to the csv containing the regression tested version results
...
if __name__ == "__main__":
    ...
    parser.add_argument('--targeted-csv', type=str)
    ...
    if args.generate_framework_def:
        ...
        assert args.owner and args.branch and args.commit

        generate_framework_def(args.owner, args.branch, args.commit)

    elif args.compare_results:

        assert all([args.baseline_csv, args.targeted_csv, args.baseline_means_to,
                    args.targeted_means_to, args.compared_means_to])

        baseline_means, targeted_means, compared_means = create_comparison(
            baseline_csv=args.baseline_csv,
            targeted_csv=args.targeted_csv,
        )
