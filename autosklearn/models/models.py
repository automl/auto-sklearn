...
class MyAutoML:
    ...
    """Rough sketch of a class that "solves" the AutoML problem. We illustrate
    various type of data that will be encountered in the challenge can be
    handled. Also, we make sure that the model regularly outputs predictions on
    validation and test data, such that, if the execution of the program is
    interrupted (timeout) there are still results provided by the program. The
    baseline methods chosen are not optimized and do not provide particularly
    good results. In particular, no special effort was put into dealing with
    ...
    def __init__(self, info, verbose=True, debug_mode=False):
        ...
        self.seed = 1
        # self.postprocessor = MultiLabelEnsemble(LogisticRegression(),
        # balance=True) # To calibrate proba
        self.postprocessor = MultiLabelEnsemble(LogisticRegression(),
                                                balance=False
                                                )  # To calibrate proba
        if debug_mode >= 2:
        ...
        if info['task'] == REGRESSION:
            if info['is_sparse'] == True:
                ...
                self.model = BaggingRegressor(
                    ...
                    n_estimators=1,
                    verbose=verbose,
                    random_state=self.seed)  # unfortunately, no warm start...
            else:
                self.name = 'GradientBoostingRegressor'
                self.model = GradientBoostingRegressor(n_estimators=1,
                                                       verbose=verbose,
                                                       warm_start=True,
            ...
            # Out of lazziness, we do not convert categorical variables...
            if info['has_categorical']:
                ...
                self.model = eval(
                    self.name + '(n_estimators=1, verbose=' + str(verbose) +
                    ', min_samples_split=10, random_state=1, warm_start = True)')
    ...
    def fit(self, X, Y):
        ...
        if self.task != REGRESSION and self.postprocessor is not None:
    ...
    def predict(self, X):
        ...
        if self.target_num == 1 and len(
                prediction.shape) > 1 and prediction.shape[1] > 1:
...
class MultiLabelEnsemble:
    ...
    def __init__(self, predictorInstance, balance=False):
        ...
        self.n_label = 1
        self.n_target = 1
        # for predictors that are ensembles_statistics of estimators
        self.n_estimators = 1
    ...
    def __str__(self):
        return 'MultiLabelEnsemble : \n' + '\tn_label={}\n'.format(
            self.n_label) + '\tn_target={}\n'.format(
                self.n_target) + '\tn_estimators={}\n'.format(
                    self.n_estimators) + str(self.predictors[0])

    def fit(self, X, Y):
        if len(Y.shape) == 1:
            # Transform vector into column matrix
            Y = np.array([Y]).transpose()
            # This is NOT what we want: Y = Y.reshape( -1, 1 ), because
            # Y.shape[1] out of range
        self.n_target = Y.shape[1]  # Num target values = num col of Y
        self.n_label = len(
            set(Y.ravel())
        )  # Num labels = num classes (categories of categorical var if n_target=1 or n_target if labels are binary )
    ...
    def predict_proba(self, X):
        ...
        # Keep only 1 prediction, 1st column = (1 - 2nd column)
        if self.n_label == 2:
            prediction = prediction[:, 1]
        for i in range(
                1,
                self.n_target):  # More than 1 target, we assume that labels are binary
