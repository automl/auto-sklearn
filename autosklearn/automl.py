    ...
    def subsample_if_too_large(
        ...
        if memory_limit and isinstance(X, np.ndarray):
            ...
            if X.dtype == np.float32:
                ...
                multiplier = 8
                logger.warning(
                    "Unknown dtype for X: %s, assuming it takes 8 bit/number",
                    str(X.dtype),
                )
            ...
            if memory_limit <= megabytes * 10 and X.dtype != np.float32:
                ...
                logger.warning(
                    ...
                    f" reducing the precision from {X.dtype} to {cast_to}",
                )
                X = X.astype(cast_to)
            ...
            if memory_limit <= megabytes * 10:
                ...
                logger.warning(
                    f"Dataset too large for memory limit {memory_limit}MB,"
                    f" reducing sample size from {X.shape[0]} to {new_num_samples}",
                )
                if task in CLASSIFICATION_TASKS:
                    # Identify if it has unique labels and allow for
                    # stratification, with unique labels in training set
                    values, idxs, counts = np.unique(
                        y, axis=0, return_index=True, return_counts=True
                    )
                    unique_labels = {
                        idx: value
