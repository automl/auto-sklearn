...
class NumberOfInstances(MetaFeature):
    def _calculate(self, X, y, logger, encoded):
...
class MissingValues(HelperFunction):
    ...
    def _calculate_sparse(self, X, y, logger, encoded):
...
class NumberOfNumericFeatures(MetaFeature):
    def _calculate(self, X, y, logger, encoded):
        return len(encoded) - np.sum(list(encoded.values()))


@metafeatures.define("NumberOfEncodedFeatures")
class NumberOfEncodedFeatures(MetaFeature):
    def _calculate(self, X, y, logger, encoded):
        return np.sum(list(encoded.values()))
...
class RatioNumericalToNominal(MetaFeature):
    def _calculate(self, X, y, logger, encoded):
        num_encoded = float(
            metafeatures["NumberOfEncodedFeatures"](X, y, logger, encoded).value
        )
        num_numerical = float(
            metafeatures["NumberOfNumericFeatures"](X, y, logger, encoded).value
        )
        if num_encoded == 0.0:
            return 0.0
        return num_numerical / num_encoded
...
class RatioNominalToNumerical(MetaFeature):
    def _calculate(self, X, y, logger, encoded):
        ...
        if num_numerical == 0.0:
            ...
            return num_encoded / num_numerical
    ...
    def _calculate(self, X, y, logger, encoded):
        if len(y.shape) == 2:
            ...
            for i in range(y.shape[1]):
                occurences.append(self._calculate(X, y[:, i], logger, encoded))
            return occurences
        else:
...
class NumSymbols(HelperFunction):
    def _calculate(self, X, y, logger, encoded):
        ...
        for i in range(X.shape[1]):
            if encoded[X.columns[i] if hasattr(X, "columns") else i]:
...
class SymbolsMean(MetaFeature):
    def _calculate(self, X, y, logger, encoded):
        # TODO: encoded attributes without a symbol don't count towards this
        # measure
...
class Kurtosisses(HelperFunction):
    def _calculate(self, X, y, logger, encoded):
        kurts = []
        for i in range(X.shape[1]):
            if not encoded[X.columns[i] if hasattr(X, "columns") else i]:
                kurts.append(
...
def calculate_all_metafeatures_encoded_labels(
    X, y, encoded, dataset_name, logger, calculate=None, dont_calculate=None
):
    """
...
def calculate_all_metafeatures(
    ...
    while len(to_visit) > 0:
        ...
        if dependency is not None:
            ...
            if is_metafeature and is_helper_function:
                ...
                value = helper_functions[dependency](
                    X_, y_, encoded=encoded_, logger=logger
                )
        ...
        value = metafeatures[name](X_, y_, logger, encoded_)
