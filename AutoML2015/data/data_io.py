...
def write(filename, predictions):
    ...
    with open(filename, "w") as output_file:
        for row in predictions:
            if type(row) is not numpy.ndarray and type(row) is not list:
                row = [row]
            for val in row:
...
def zipdir(archivename, basedir):
    ...
    with contextlib.closing(ZipFile(archivename, "w", ZIP_DEFLATED)) as z:
...
def inventory_data_nodir(input_dir):
    ...
    for i in range(0, len(training_names)):
        ...
        training_names[i] = name[-name[::-1].index(filesep):-name[::-1].
                                 index('_')-1]
...
def data_binary_sparse(filename, feat_type):
    ...
    inner_data = data_converter.file_to_array(filename)
...
def copy_results(datanames, result_dir, output_dir, verbose):
    # This function copies all the [dataname.predict] results from result_dir
    # to output_dir
    for basename in datanames:
        try:
            test_files = glob.glob(result_dir + "/" + basename + "*_test*.predict")
            if len(test_files) == 0:
                vprint(verbose, "[-] Missing 'test' result files for " +
                       basename)
                return 0
            for f in test_files:
                shutil.copy2(f, output_dir)
            valid_files = glob.glob(result_dir + "/" + basename + "*_valid*.predict")
            if len(valid_files) == 0:
                vprint(verbose, "[-] Missing 'valid' result files for " +
                       basename)
                return 0
...
def show_io(input_dir, output_dir):     
    ...
    write_list(glob.glob(input_dir))
    write_list(glob.glob(input_dir + '/*'))
    write_list(glob.glob(input_dir + '/*/*'))
    write_list(glob.glob(input_dir + '/*/*/*'))
    swrite("\n")
    swrite("-- Output directory  " + output_dir + ":\n")
    write_list(glob.glob(output_dir))
    write_list(glob.glob(output_dir + '/*'))
    swrite("\n")
